...
public static class MECExtensionMethods2
{
    ...
    {
        while (FLCore.Timing.MainThread != System.Threading.Thread.CurrentThread || (IsNodeAlive(node) && coroutine.MoveNext()))
    ...
    {
        while (FLCore.Timing.MainThread != System.Threading.Thread.CurrentThread || (IsNodeAlive(node1) && IsNodeAlive(node2) && coroutine.MoveNext()))
            yield return coroutine.Current;
    }
    ...
    {
        while (FLCore.Timing.MainThread != System.Threading.Thread.CurrentThread || (IsNodeAlive(node1) && IsNodeAlive(node2) && IsNodeAlive(node3) && coroutine.MoveNext()))
            yield return coroutine.Current;
    }
}
